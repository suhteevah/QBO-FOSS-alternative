...
def upgrade() -> None:
    ...
    # ── Users ──────────────────────────────────────────────
    user_role = sa.Enum("admin", "accountant", "client", name="userrole")
    op.create_table(
        "users",
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
    ...
    account_subtype = sa.Enum(
        "cash", "accounts_receivable", "inventory", "prepaid",
        "fixed_asset", "other_asset",
        "accounts_payable", "credit_card", "accrued_liability",
        "long_term_debt", "other_liability",
        "owners_equity", "retained_earnings",
        "sales", "service_revenue", "other_income",
        "cost_of_goods", "operating_expense", "payroll",
        "tax_expense", "depreciation", "other_expense",
        name="accountsubtype",
    )
    op.create_table(
        "accounts",
        ...
        sa.Column("account_subtype", account_subtype),
    ...
    op.create_unique_constraint("uq_org_account_number", "accounts", ["organization_id", "account_number"])
    op.create_index("ix_account_type", "accounts", ["organization_id", "account_type"])

    # ── Receipts (must exist before journal_entries FK) ────
    op.create_table(
        "receipts",
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("organization_id", UUID(as_uuid=True), sa.ForeignKey("organizations.id"), nullable=False),
        ...
        sa.Column("line_items_json", JSONB()),
    ...
    op.create_table(
        ...
        sa.Column("suggested_account_id", UUID(as_uuid=True), sa.ForeignKey("accounts.id"), nullable=True),
        sa.Column("ai_confidence", sa.Numeric(5, 4)),
    ...
    # ── Accounting Periods ─────────────────────────────────
    period_status = sa.Enum("open", "closed", name="periodstatus")
    op.create_table(
        "accounting_periods",
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("organization_id", UUID(as_uuid=True), sa.ForeignKey("organizations.id"), nullable=False),
    ...
    op.create_unique_constraint("uq_org_period", "accounting_periods", ["organization_id", "start_date", "end_date"])
    ...
    op.create_table(
        ...
        sa.Column("response_structured", JSONB()),
...
def downgrade() -> None:
    ...
    sa.Enum(name="accountsubtype").drop(op.get_bind(), checkfirst=True)
    sa.Enum(name="transactionsource").drop(op.get_bind(), checkfirst=True)
